...
RegistryRequirement = { name: RegistryName, version: RegistryVersion }
RegistryIdentity = {
  name: RegistryName,
  version: RegistryVersion,
  fingerprint: ContentHash,
  advisoryFingerprint: ContentHash
}
Deprecation = {
  since: RegistryVersion,
  message: String,
  replacement?: String,
  removal?: RegistryVersion
}

LanguageEnvironmentResolver = {
  resolve(RegistryRequirement) -> LanguageEnvironment | UnknownRegistry | UnknownVersion
  availableVersions(RegistryName) -> [RegistryIdentity]
}
...
TOWL defines application syntax and algebraic contracts, but it defines **no mandatory function or aggregator names**. Each runtime chooses its vocabulary. Every document names the exact language environment it requires through `plan.registry`. A processor MUST resolve that requirement before parsing registry-dependent expression and aggregator applications, then use the same immutable environment for schema generation, validation reporting, and execution.

Registry resolution rules are normative:

1. `name` and `version` are both required opaque identifiers. Version ranges, aliases such as `latest`, and implicit current-version selection are invalid in v1.
2. Resolution is exact. A runtime MAY retain several versions concurrently; it MUST NOT substitute a newer, older, or similarly named version.
3. A published `(name, version)` is semantically immutable. Reinstalling the same pair with different signatures, evaluators, context dependencies, or laws is a registry-integrity error, not a new version. Deprecation metadata is a non-semantic advisory snapshot: it MAY gain or refine migration guidance without changing behavior, and its independent `advisoryFingerprint` changes when it does.
4. An unavailable name or version is a validation error. The diagnostic SHOULD list available names or versions, but execution never falls back.
5. The validated plan stores the resolved `LanguageEnvironment`; execution performs no registry name or overload lookup.

`RegistryIdentity.fingerprint` MUST be a deterministic content hash of every semantic function/aggregator descriptor and law that can affect validation or execution; it excludes advisory deprecation fields. `advisoryFingerprint` MUST hash the complete selected environment/function/aggregator advisory snapshot, including the empty set when nothing is deprecated. Both fingerprints MUST be recorded in every execution result. A saved document remains runnable when its exact registry version remains installed; its requirement does not change when another version becomes current.

A document is portable only to runtimes that install its exact registry requirement and whose operation catalog satisfies its calls. Unknown names fail validation; they never become dynamic calls.
...
The validated plan stores `OperationId` and `InvocationHandle`; execution does not repeat string lookup. The validation report SHOULD show the resolved origin even when `service` was omitted.
...
- be visible in validation and its report;
...
A plan is a versioned block with an exact language-registry requirement and optional declared inputs.

```text
Plan = { towl, registry, description, inputs? } & Block
```
...
{
  "towl": "v1",
  "registry": {"name": "example", "version": "1"},
  "description": "example",
  "result": "ok"
}
```
...
- Body cardinality is preserved as the body value, not flattened: `One<U>` contributes one `U`; `Optional<U>` contributes zero or one; `Many<U>` contributes one ordered `List<U>` for that outer element. Therefore a Many-valued body makes the traversal element type `List<U>` and the total value `Many<List<U>>`.
- Under `collect`, each successful body value—including a list from a Many-valued body—is wrapped once as `{"ok": value}`; each failed outer element contributes one error tag.
...
`ok` and `error` are member names inside collect elements only; they are not reserved traversal keys, and the input element is not echoed automatically outside `error.item`. Traversal never performs implicit `flatMap`; a processor SHOULD warn when a Many-valued body creates `Many<List<U>>` unless the surrounding shape makes that nesting clearly intentional. A body `result` that needs its input includes it explicitly, since the traversal name is in scope. The `error` object MUST contain `item`, `code`, and `message`; hosts may add diagnostic members.
...
- every algebraic law or semantic role used by normalization, correlation, or optimization; and
- structured deprecation metadata when the function is deprecated.

The evaluator MUST NOT read ambient state, perform I/O, invoke tools, mutate values, or observe scheduling. A function SHOULD receive host context through explicit `env` arguments. If a runtime registers an implicit immutable context dependency, the descriptor MUST declare it; dependency extraction, the validation report, result context, and registry identity MUST include it.
...
- deprecation status, replacement guidance, and planned removal versions;
...
An aggregator that presents the source elements as a list preserves each element exactly and adds one list layer; it is not a flattening operator unless its registered descriptor explicitly defines and types flattening semantics. TOWL v1 has no implicit flatten conversion. Aggregators consume an Express stream and are invalid in Assemble results, which have no implicit stream.
...
A runtime MUST expose aggregator names, signatures, input/output types, law metadata, and deprecation status for schema generation, authoring guidance, validation reporting, and conformance testing. TOWL defines no standard aggregator list.
...
Registry deprecations are non-semantic structured warnings, not prose-only documentation. The resolver freezes one advisory snapshot for the validation/execution lifecycle even if installed advisory data is updated concurrently.

```text
DeprecationDiagnostic = {
  code: registry.deprecated | function.deprecated | aggregator.deprecated,
  severity: warning,
  path: Path,
  since: RegistryVersion,
  message: String,
...
Validation MUST emit exactly one deduplicated diagnostic for the selected deprecated language environment and exactly one for each deprecated function or aggregator descriptor referenced by the document. These diagnostics MUST also appear in the validation report and in `ExecutionResult.diagnostics`, including when execution otherwise succeeds. They use stable codes `registry.deprecated`, `function.deprecated`, and `aggregator.deprecated`, severity `warning`, the authored path or registry requirement, `since`, `message`, and any `replacement` or `removal` value. Deprecation never silently rewrites a name and does not change semantics. If a descriptor or registry version has actually been removed, ordinary exact-resolution failure applies; a host policy MAY reject a deprecated dependency as a distinct policy decision.
...
### 13.5 Validation report model

Validation returns one static review artifact and invokes no operations. On success the report exposes at least:
...
ValidationReport = {
  ...
  diagnostics: [Diagnostic],
  registryIdentity: RegistryIdentity,
  operations: [ResolvedOperationSummary],
  symbols: SymbolTable,
  dependencyWaves: [Wave],
  effects: EffectSet,
...
The report describes resolved registry identity/fingerprints/deprecations, operation identities and effects, inferred symbols/cardinalities/shapes, dependency waves, traversal/correlation structure, paging and other capabilities, logical or symbolic work bounds, optimization classifications, and host policy decisions. It SHOULD emit structured warnings when traversal creates nested collection cardinality or a list-producing aggregator adds another list layer, because both states are valid but commonly mistaken for flattening. An invalid document returns `valid: false` with diagnostics and any safely resolved context, but never a validated plan. The language does not prescribe a command or rendering; hosts SHOULD make this report the sole static review result rather than introducing a separate dry-run phase.
...
4. the exact registry requirement and every operation resolve uniquely and are frozen in the validated plan, with no version fallback;
...
20. execution always consumes a validated immutable plan and reports every referenced deprecation.
...
- traversal output shape under each error policy, including tagged collect elements and Many-valued body nesting;
- list-producing aggregators preserve list-valued elements and never flatten implicitly;
...
- exact registry-version selection with multiple installed versions, unavailable-version failure, no fallback, and immutable semantic `(name, version)` fingerprints;
- advisory updates leave the semantic fingerprint and execution behavior unchanged while changing `advisoryFingerprint`;
- structured, deduplicated registry/function/aggregator deprecation diagnostics in the validation report and execution;
...
LanguageEnvironment:
  identity = {name: "example-tools", version: "1", fingerprint: "sha256:...", advisoryFingerprint: "sha256:..."}
...
{
  "towl": "v1",
  "registry": {"name": "example-tools", "version": "1"},
...
- Plans select an exact immutable language-registry name and version. Execution-result fingerprints alone were insufficient for saved plans: they proved what ran after resolution but could not select an older installed vocabulary after a new version appeared.
- Deprecation became descriptor data and structured diagnostics rather than prose, so saved-plan review and automation can identify registry, function, and aggregator migrations without changing execution semantics.
